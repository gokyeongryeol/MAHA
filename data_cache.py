...
class GaussianProcess(object):
    ...
    def generate_batch(self, batch_size, noise, is_test):
        num_context = np.random.randint(5, 10)
        num_target = np.random.randint(10, 20-num_context)
        
        length = np.random.uniform(1.0, 2.0, size=(batch_size,1,1,1))
...
if __name__ == '__main__':
    ...
    for data in ['mixture', 'GP']:
        for i, (is_train, is_test) in enumerate(zip([True, True, False], [False, True, True])):
            if data == "mixture":
                loader = Heterogeneous_dataset('1D', is_train=is_train)
            else:
                loader = GaussianProcess(is_train=is_train)

            que_len, batch_size = [100000, 1000, 1][i], [100, 100, 1][i]
